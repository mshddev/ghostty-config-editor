...
/// Three pieces: `SurfaceHeader` (top), `SurfaceFeedbackBar`
...
/// consumes, so no view hand-picks a literal. The primitives are framework-
...
enum DesignTokens {
    ...
    /// The background lift a control gains on hover / keyboard focus.
    static let hoverLift = Color.primary.opacity(0.06)
}

/// Named durations + curves for the app's small motion vocabulary. Transitions
...
/// The two-step semantic type scale: `surfaceTitle` is the routine per-surface
/// header; `heroTitle` is the one reserved larger step for the single identity moment
/// (Welcome), so it visibly outranks ordinary chrome.
extension Font {
...
/// The one hover/focus affordance for strengthenable
/// controls — the ⓘ / reset icon buttons, the Advanced disclosure header, the Welcome
/// jump-in cards, the Find result rows. It lifts the background by the `hoverLift`
/// token on pointer hover **and** on keyboard focus, identically, so no affordance is
/// pointer-only (the parity rule). It changes
...
/// Icon buttons that already size their own hit target use `.icon` (zero insets).
...
/// a theme's "Current", a Find result's category, a keybind's origin. Replaces six
...
struct Pill: View {
    ...
    var body: some View {
        ...
        // A state-triggered pill (a theme's "Current") scales in rather than
...
/// — for narrowing a long list to a single group. Shared chrome so the Keyboard
    ...
    private func pill(title: String, isSelected: Bool, action: @escaping () -> Void) -> some View {
        Button(action: action) {
            Text(title)
                ...
                // accent in white (accent reserved for selection) — a clear, tappable
...
/// macOS grouped Forms drop `role: .destructive`-only styling. The reset-all rows
/// consume it.
...
/// overlay: a magnifying-glass icon, a plain text field, and a clear button on
/// the neutral `subtleFill`. An optional `FocusState` binding lets Find drive
/// focus-on-appear (and make the per-surface field keyboard-reachable).
struct SurfaceSearchField: View {
    ...
    /// itself. It yields to the clear button the moment the user types — the two are
...
/// inline per-row feedback map a state the same way instead of drifting.
...
extension AppModel.ApplyState {
    ...
    /// animates in. `.applying` is *not* settled, so it appears instantly.
    var isSettled: Bool {
        switch self {
...
/// The one visual for a settled apply state: the icon+headline line over the
/// stacked captions (notice, auto-reload message, git-tracked hint). The per-row
/// feedback and the surface feedback bar both render this identically — each owns
    ...
    var body: some View {
        ...
        case .succeeded(let headline, let notice, let reload):
            VStack(alignment: .leading, spacing: 2) {
                ...
                // the source so a happy-path save reads just "Saved · Undo".
...
/// The failure line: the normalized plain-language `message` as the headline,
...
/// means two things at once (the global ⇧⌘F Find is a separate affordance).
...
/// search without the mouse — the keyboard gap System Settings never closed.
...
struct StatusBackLink: View {
    ...
    var body: some View {
        HStack {
            // Back to the Status hub via the destination model; the sidebar keeps
...
/// `keybind` → the Keyboard Shortcuts surface (Recommended, Customized). This
    ...
    var body: some View {
        ...
        case .failed(let presentation):
            bar {
                HStack(alignment: .top, spacing: 8) {
                    ApplyFeedbackContent(state: applyState)
                    // Stale-on-disk: the fix is a reload, so offer it right on the banner.
...
/// A "jump-in" springboard card: an accent icon, a title, an optional one-line
/// detail, and a trailing chevron, styled as a pickable row with the hover/focus
